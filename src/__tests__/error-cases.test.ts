...
describe("Error Handling and Edge Cases", () => {
  describe("Malformed Headers", () => {
    ...
    it("should throw error for wrong header signature", () => {
      ...
      expect(() => parseMidi(wrongHeader)).toThrow(
    ...
    it("should throw error for invalid header length", () => {
      const wrongLength = new Uint8Array([
        ...
        0x00,
        0x00,
        0x08, // Wrong length (8 instead of 6)
        0x00,
        0x00,
        0x00,
        0x01,
      ...
      expect(() => parseMidi(wrongLength)).toThrow(
    ...
    it("should throw error for unsupported format types", () => {
      ...
      for (const format of formats) {
        ...
        expect(() => parseMidi(data)).toThrow(
          `Unsupported MIDI format: ${format}`,
        );
      }
    });
  });
  ...
  describe("Malformed Tracks", () => {
    ...
    it("should handle tracks longer than declared length", () => {
      const data = new Uint8Array([
        ...
        0xff,
        0x2f,
        0x00, // End of track (4 bytes)
        0x00,
        0x90,
        0x40,
        0x7f, // Extra data (should not be parsed)
      ]);
  ...
  describe("Running Status Errors", () => {
    it("should throw error when running status used without previous status", () => {
      const data = new Uint8Array([
        ...
        0x00, // Delta time 0
        0x40,
        0x7f, // Invalid: data bytes without status
        0x00,
        0xff,
        0x2f,
        0x00, // End of track
    ...
    it("should handle running status after meta events correctly", () => {
      const data = new Uint8Array([
        ...
        0x90,
        0x40,
        0x7f, // Note On (establishes running status)
        0x00, // Delta time 0
        0xff,
        0x51,
        0x03,
        0x07,
        0xa1,
        0x20, // Meta event (should clear running status)
        0x00, // Delta time 0
        0x80,
        0x40,
        0x40, // Need explicit status after meta event
        0x00,
        0xff,
        0x2f,
        0x00, // End of track
  ...
  describe("Invalid Event Data", () => {
    it("should throw error for unsupported event status bytes", () => {
      ...
      for (const status of invalidStatuses) {
        ...
        expect(() => parseMidi(data)).toThrow(
          `Unsupported event status: 0x${status.toString(16)}`,
        );
      }
    });
    ...
    it("should handle incomplete channel events by throwing error", () => {
      const data = new Uint8Array([
        ...
        0x00, // Delta time 0
        0x90,
        0x40, // Note On with missing velocity byte
        0x00,
        0xff,
        0x2f,
        0x00, // End of track
    ...
    it("should handle incomplete meta events by throwing error", () => {
      const data = new Uint8Array([
        ...
        0xff,
        0x03,
        0x10, // Text meta event claiming 16 bytes
        0x41,
        0x42, // But only 2 bytes available
      ]);
    ...
    it("should handle incomplete SysEx events by throwing error", () => {
      const data = new Uint8Array([
        ...
        0x00, // Delta time 0
        0xf0,
        0x10, // SysEx claiming 16 bytes
        0x43,
        0x12, // But only 2 bytes available
      ]);
  ...
  describe("Boundary Conditions", () => {
    ...
    it("should handle edge case channel and note values", () => {
      const data = new Uint8Array([
        ...
        0x9f,
        0x7f,
        0x7f, // Note On: channel 15, note 127, velocity 127 (all maximum values)
        0x00, // Delta time 0
        0x80,
        0x00,
        0x00, // Note Off: channel 0, note 0, velocity 0 (all minimum values)
        0x00, // Delta time 0
        0xef,
        0x7f,
        0x7f, // Pitch Bend: channel 15, max positive value
        0x00,
        0xff,
        0x2f,
        0x00, // End of track
  ...
  describe("Memory and Performance Edge Cases", () => {
    ...
    it("should handle files with many short events efficiently", () => {
      ...
      const data = new Uint8Array([
        ...
        0x6b, // "MTrk"
        ...[
          (eventData.length >> 24) & 0xff,
          (eventData.length >> 16) & 0xff,
          (eventData.length >> 8) & 0xff,
          eventData.length & 0xff,
        ],
        ...eventData,
      ]);
