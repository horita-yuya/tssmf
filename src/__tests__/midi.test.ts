...
import { parseMidi, buildTempoMap, ticksToMs, type MidiFile, type ChannelEvent, type MetaEvent, type SysExEvent } from '../midi';
...
describe('MIDI Parser', () => {
  ...
  beforeAll(() => {
    ...
    const testMidiPath = '/Users/yuyahorita/oss/midi-ts/test_simple.mid';
  ...
  describe('MIDI Header Parsing', () => {
    ...
    it('should parse SMPTE division correctly', () => {
      const headerData = new Uint8Array([
        ...
        0x00, 0x00,             // Format 0
        0x00, 0x01,             // 1 track
        0xE7, 0x28,             // SMPTE: -25 fps, 40 ticks per frame
        0x4D, 0x54, 0x72, 0x6B, // "MTrk"
        0x00, 0x00, 0x00, 0x04, // Track length
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should throw error for invalid header', () => {
      const invalidHeader = new Uint8Array([
        0x4D, 0x54, 0x68, 0x65, // "MThe" (invalid)
        0x00, 0x00, 0x00, 0x06,
        0x00, 0x00, 0x00, 0x01, 0x01, 0xE0
      ]);

      expect(() => parseMidi(invalidHeader)).toThrow('Invalid MIDI file: missing MThd header');
    });
    ...
    it('should throw error for unsupported format', () => {
      ...
      expect(() => parseMidi(unsupportedFormat)).toThrow('Unsupported MIDI format: 3');
    });

    it('should throw error for invalid header length', () => {
      const invalidLength = new Uint8Array([
        0x4D, 0x54, 0x68, 0x64, // "MThd"
        0x00, 0x00, 0x00, 0x08, // Invalid length (8 instead of 6)
        0x00, 0x00, 0x00, 0x01, 0x01, 0xE0, 0x00, 0x00
      ]);

      expect(() => parseMidi(invalidLength)).toThrow('Invalid MIDI header length');
    });
  });
  ...
  describe('Variable Length Quantity (VLQ) Parsing', () => {
    it('should parse single-byte VLQ values', () => {
      ...
      const data = new Uint8Array([
        ...
        0x00,                   // Delta time 0
        0xFF, 0x2F, 0x00        // End of track (duplicate for length)
      ]);
    ...
    it('should parse multi-byte VLQ values', () => {
      const data = new Uint8Array([
        ...
        0x4D, 0x54, 0x72, 0x6B, // "MTrk"
        0x00, 0x00, 0x00, 0x09,
        0x81, 0x00,             // Delta time 128 (two bytes: 0x81 0x00)
        0xFF, 0x2F, 0x00,       // End of track
    ...
    it('should parse large VLQ values', () => {
      const data = new Uint8Array([
        ...
        0x4D, 0x54, 0x72, 0x6B, // "MTrk"
        0x00, 0x00, 0x00, 0x0B,
        0x83, 0xFF, 0x7F,       // Delta time 65535 (three bytes)
        0xFF, 0x2F, 0x00,       // End of track
        0x00, 0xFF, 0x2F, 0x00  // Padding
      ]);
  ...
  describe('Channel Events', () => {
    it('should parse Note On events', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0A,
        0x00,                   // Delta time 0
        0x90, 0x40, 0x7F,       // Note On: channel 0, note 64 (middle C), velocity 127
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should parse Note Off events', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0A,
        0x00,                   // Delta time 0
        0x80, 0x40, 0x40,       // Note Off: channel 0, note 64, velocity 64
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should parse Polyphonic Aftertouch events', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0A,
        0x00,                   // Delta time 0
        0xA5, 0x40, 0x50,       // Poly Aftertouch: channel 5, note 64, pressure 80
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should parse Control Change events', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0A,
        0x00,                   // Delta time 0
        0xB2, 0x07, 0x64,       // Control Change: channel 2, controller 7 (volume), value 100
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should parse Program Change events', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x09,
        0x00,                   // Delta time 0
        0xC3, 0x19,             // Program Change: channel 3, program 25
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should parse Channel Pressure events', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x09,
        0x00,                   // Delta time 0
        0xD7, 0x60,             // Channel Pressure: channel 7, pressure 96
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should parse Pitch Bend events', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0A,
        0x00,                   // Delta time 0
        0xE4, 0x00, 0x40,       // Pitch Bend: channel 4, LSB=0, MSB=64 (center position)
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should parse Pitch Bend with maximum positive value', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0A,
        0x00,                   // Delta time 0
        0xE0, 0x7F, 0x7F,       // Pitch Bend: channel 0, LSB=127, MSB=127 (max positive)
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should parse Pitch Bend with maximum negative value', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0A,
        0x00,                   // Delta time 0
        0xE0, 0x00, 0x00,       // Pitch Bend: channel 0, LSB=0, MSB=0 (max negative)
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
  ...
  describe('Running Status', () => {
    it('should handle running status for channel events', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x10,
        0x00,                   // Delta time 0
        0x90, 0x40, 0x7F,       // Note On: channel 0, note 64, velocity 127
        0x30,                   // Delta time 48
    ...
    it('should throw error when running status used without previous status', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x08,
        0x00,                   // Delta time 0
        0x40, 0x7F,             // Invalid: running status without previous status
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should reset running status for meta and sysex events', () => {
      const data = new Uint8Array([
        ...
        0x90, 0x40, 0x7F,       // Note On: channel 0, note 64, velocity 127
        0x00,                   // Delta time 0
        0xFF, 0x51, 0x03, 0x07, 0xA1, 0x20, // Set Tempo meta event
        0x00,                   // Delta time 0
        0x80, 0x40, 0x40,       // Note Off (new status required after meta event)
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
  ...
  describe('Meta Events', () => {
    it('should parse text meta events', () => {
      ...
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x11,
        0x00,                   // Delta time 0
        0xFF, 0x03, 0x0A,       // Text meta event type 3, length 10
    ...
    it('should parse tempo meta events', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0A,
        0x00,                   // Delta time 0
        0xFF, 0x51, 0x03,       // Set Tempo meta event, length 3
        0x07, 0xA1, 0x20,       // 500,000 microseconds per quarter note (120 BPM)
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should parse time signature meta events', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0B,
        0x00,                   // Delta time 0
        0xFF, 0x58, 0x04,       // Time Signature meta event, length 4
        0x04, 0x02, 0x18, 0x08, // 4/4 time, metronome=24, thirty-seconds=8
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should parse unknown meta events with raw data', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0A,
        0x00,                   // Delta time 0
        0xFF, 0x7F, 0x03,       // Unknown meta event type 0x7F, length 3
  ...
  describe('SysEx Events', () => {
    it('should parse standard SysEx events (0xF0)', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0C,
        0x00,                   // Delta time 0
        0xF0, 0x05,             // SysEx F0, length 5
        0x43, 0x12, 0x00, 0x43, 0xF7, // SysEx data ending with F7
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should parse continuation SysEx events (0xF7)', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0A,
        0x00,                   // Delta time 0
        0xF7, 0x03,             // SysEx F7 (continuation), length 3
        0x01, 0x02, 0x03,       // Continuation data
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
  ...
  describe('Track Parsing', () => {
    it('should throw error for invalid track header', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x06,
        0x00, 0x00, 0x00, 0x01, 0x01, 0xE0,
        0x4D, 0x54, 0x72, 0x6C, // "MTrl" (invalid)
        0x00, 0x00, 0x00, 0x04,
        0x00, 0xFF, 0x2F, 0x00
      ]);
    ...
    it('should parse empty tracks', () => {
      const data = new Uint8Array([
        ...
        0x4D, 0x54, 0x72, 0x6B, // "MTrk"
        0x00, 0x00, 0x00, 0x04,
        0x00, 0xFF, 0x2F, 0x00  // Only end of track
      ]);
    ...
    it('should stop parsing track at end of track event', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0A,
        0x00, 0xFF, 0x2F, 0x00, // End of track
        0x00, 0x90, 0x40, 0x7F  // This should not be parsed
      ]);
  ...
  describe('Error Handling', () => {
    ...
    it('should handle truncated data gracefully', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x01, 0x01, 0xE0,
        0x4D, 0x54, 0x72, 0x6B, // "MTrk"
        0x00, 0x00, 0x00, 0x10, // Claims 16 bytes but only provides 4
        0x00, 0xFF, 0x2F, 0x00
      ]);
    ...
    describe('buildTempoMap', () => {
      it('should create default tempo map with no tempo events', () => {
        const midi: MidiFile = {
          ...
          tracks: [{
            events: [
              { delta: 0, type: 'meta', metaType: 0x2F, data: new Uint8Array(), endOfTrack: true }
            ]
          }],
          ticksPerQuarter: 480
        };
      ...
      it('should build tempo map with single tempo change', () => {
        const midi: MidiFile = {
          ...
          tracks: [{
            events: [
              { delta: 480, type: 'meta', metaType: 0x51, data: new Uint8Array([0x06, 0x1A, 0x80]), tempoUsPerQuarter: 400000 },
              { delta: 0, type: 'meta', metaType: 0x2F, data: new Uint8Array(), endOfTrack: true }
            ]
          }],
          ticksPerQuarter: 480
      ...
      it('should build tempo map with multiple tempo changes', () => {
        const midi: MidiFile = {
          ...
          tracks: [
            {
              events: [
                { delta: 0, type: 'meta', metaType: 0x51, data: new Uint8Array(), tempoUsPerQuarter: 600000 },
                { delta: 960, type: 'meta', metaType: 0x51, data: new Uint8Array(), tempoUsPerQuarter: 300000 },
                { delta: 0, type: 'meta', metaType: 0x2F, data: new Uint8Array(), endOfTrack: true }
              ]
            },
            {
              events: [
                { delta: 480, type: 'meta', metaType: 0x51, data: new Uint8Array(), tempoUsPerQuarter: 450000 },
                { delta: 0, type: 'meta', metaType: 0x2F, data: new Uint8Array(), endOfTrack: true }
              ]
            }
          ],
      ...
      it('should handle tempo changes at the same tick', () => {
        const midi: MidiFile = {
          ...
          tracks: [{
            events: [
              { delta: 480, type: 'meta', metaType: 0x51, data: new Uint8Array(), tempoUsPerQuarter: 400000 },
              { delta: 0, type: 'meta', metaType: 0x51, data: new Uint8Array(), tempoUsPerQuarter: 350000 },
              { delta: 0, type: 'meta', metaType: 0x2F, data: new Uint8Array(), endOfTrack: true }
            ]
          }],
          ticksPerQuarter: 480
  ...
  describe('Edge Cases', () => {
    it('should handle MIDI files with large track counts', () => {
      ...
      const headerData = [
        ...
        0x00, 0x00, 0x00, 0x06, // Header length
        0x00, 0x01,             // Format 1
        (numTracks >> 8) & 0xFF, numTracks & 0xFF, // Track count
        0x01, 0xE0              // 480 PPQ
      ];
      ...
      const fullData = new Uint8Array(headerData.length + (trackData.length * numTracks));
    ...
    it('should handle Note On with velocity 0 as Note Off equivalent', () => {
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0A,
        0x00,                   // Delta time 0
        0x90, 0x40, 0x00,       // Note On with velocity 0 (equivalent to Note Off)
        0x00, 0xFF, 0x2F, 0x00  // End of track
      ]);
    ...
    it('should handle very large VLQ values', () => {
      const data = new Uint8Array([
        ...
        0x4D, 0x54, 0x72, 0x6B, // "MTrk"
        0x00, 0x00, 0x00, 0x0D,
        0x8F, 0xFF, 0xFF, 0x7F, // VLQ: maximum 4-byte value (0x0FFFFFFF)
        0xFF, 0x2F, 0x00,       // End of track
        0x00, 0xFF, 0x2F, 0x00  // Padding
      ]);
    ...
    it('should preserve exact byte data in meta events', () => {
      ...
      const data = new Uint8Array([
        ...
        0x00, 0x00, 0x00, 0x0C,
        0x00,                   // Delta time 0
        0xFF, 0x7E, 0x05,       // Custom meta event, length 5
